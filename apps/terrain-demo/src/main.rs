...
use point_workspace::{
    CommitLimits, CommitOutcome, CommitRequest, OpenLimits, OperationId, PointSetLimits,
    RevisionId, Workspace, WorkspaceSchema,
};
...
fn run(command: RunCommand) -> AppResult<()> {
    ...
    let (workspace, workspace_disposition) = open_or_create_workspace(index, &command.workspace)?;
    ...
    if let Some(ordinal) = command.correction_revert_ordinal {
        surface = exercise_classification_correction_and_revert(&workspace, &surface, ordinal)?;
    }
...
fn exercise_classification_correction_and_revert(
    ...
    let correction_revision = committed_revision(
        workspace
            .commit(
    ...
    let revert_revision = committed_revision(
        workspace
            .commit(
...
fn committed_revision(
    ...
    match outcome {
        ...
        CommitOutcome::Rejected(reason) => Err(io::Error::other(format!(
            "{action} Operation {operation} was rejected: {reason:?}"
        ))
        .into()),
        CommitOutcome::Indeterminate(uncertainty) => Err(io::Error::other(format!(
            "{action} Operation {operation} is indeterminate at {:?}; reopen the Workspace and resolve this recorded Operation Identity before retrying",
            uncertainty.phase(),
        ))
        .into()),
    }
